...
This skill (re)generates `BENCHMARK.md` for **thunder-java** so every plugin's report shares the
...
```
cd <plugin>
node engine/thunder.mjs build realdemo --force      # realistic bench bed (generate it first if missing: node engine/tools/gen-realdemo.mjs realdemo && node engine/tools/populate-realdemo.mjs realdemo)
...
```
node engine/tools/token-bench.mjs realdemo          # two-tier / inline A·B·C — data tokens
node engine/tools/sweep-bench.mjs realdemo          # ≥50 routed questions → wins/total + % saved
node engine/tools/tier3-bench.mjs demo                  # answer-cache hit + prune gains
node engine/tools/analyze.mjs realdemo            # architecture / security insights
```
...
2. **Symbol** — `node engine/thunder.mjs sym def <Type> realdemo` and `sym refs <Type>`  vs grep+open.
3. **Discovery** — `grep -i <term> <cache>/capability-map.yaml`  vs reading the repo.
4. **Flow + rules** — `node engine/thunder.mjs ask "<feature> flow" realdemo`  vs reading the feature.
...
`<cache>` = `realdemo/.claude/cache/thunder-java`.
...
**Test beds.** `demo` (toy) · `realdemo` (realistic, main bed) · `bigdemo` (extreme scale).
