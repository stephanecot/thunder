...
> **Prerequisite:** opt the project in first — run `/thunder-java:thunder-java-init` once (it writes the
...
4. **Infer in batches** — split `contexts` into groups of **~10**. For each group, spawn ONE
   **thunder-java-cartographer** sub-agent (Task, `subagent_type: "thunder-java-cartographer"`), passing it
