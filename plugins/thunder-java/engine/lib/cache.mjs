...
export const cacheDir = (root) => join(root, '.thunder', 'java');

/** One-time migration: drop the pre-relocation derived index (.claude/cache/thunder-java/).
 * Projects indexed before the move to .thunder/java/ keep a stale copy there that masquerades
 * as the live index — remove it on every build. */
export function sweepLegacyCache(root) {
  for (const name of ['thunder-java', 'thunder']) {
