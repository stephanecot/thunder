---
name: thunder-angular-benchmark
...
This skill (re)generates `BENCHMARK.md` for **thunder-angular** so every plugin's report shares the
...
```
cd <plugin>
node engine/thunder.mjs build ngdemo --force      # realistic bench bed (generate it first if missing: node engine/tools/gen-ngdemo.mjs ngdemo 40)
...
```
node engine/tools/token-bench.mjs ngdemo          # two-tier / inline A·B·C — data tokens
node engine/tools/sweep-bench.mjs ngdemo          # ≥50 routed questions → wins/total + % saved
node engine/tools/tier3-bench.mjs demo                  # answer-cache hit + prune gains
node engine/tools/data-bench.mjs demo                 # 5 frozen questions × 3 tiers
```
...
2. **Symbol** — `node engine/thunder.mjs sym def <Type> ngdemo` and `sym refs <Type>`  vs grep+open.
3. **Discovery** — `grep -i <term> <cache>/capability-map.yaml`  vs reading the repo.
4. **Flow + rules** — `node engine/thunder.mjs ask "<feature> flow" ngdemo`  vs reading the feature.
...
`<cache>` = `ngdemo/.claude/cache/thunder-angular`.
...
**Test beds.** `demo` (toy) · `ngdemo` (realistic, main bed) · `ngdemo` (extreme scale).
