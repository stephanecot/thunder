...
> **Prerequisite:** opt the project in first — run `/thunder-angular:thunder-angular-init` once (it writes the committed `.thunder/angular/config.yaml` marker). Running this skill also writes that marker when it builds a non-empty index, so reindex works standalone too.
...
4. **Infer in batches** — split `contexts` into groups of **~10**. For each group spawn ONE
   **thunder-angular-cartographer** (Task, `subagent_type: "thunder-angular-cartographer"`), passing only the ids + paths of that group:
...
5. **Persist the batch in one call** — concatenate every array the cartographer returned into a single JSON
   ...
   → `{set, failed}` (re-emits shards once). If a group returned non-JSON, retry it once at half size; otherwise drop
   those contexts (they stay stale for the next run) and report them.
