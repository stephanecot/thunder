...
export function emit(root, model, functional = {}) {
  ...
  // global route table — ONE grep-friendly line per route (was 5-6 lines each); flow lives in shards
  const rtLine = (r) => `${r.path || '/'} → ${r.target || '-'}  ${r.kind}${r.guards?.length ? `  guards: ${r.guards.join(', ')}` : ''}  (${r.ctx})`;
  if (writeIfChanged(join(dir, 'routes.yaml'),
    dump({ format: 'path → target  kind  [guards: g1, g2]  (context)', routes: model.routes.map(rtLine) }))) changed++;
