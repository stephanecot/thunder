...
const INIT_CONFIG = `# thunder-angular — committed project marker. Its presence turns indexing ON for this project.
# Without it, thunder-angular stays completely idle (no .thunder/angular/ dir, no tokens spent).
enabled: true
language: angular
`;

// Opt-in marker: thunder-angular only indexes a project once `init` has been run there
// (committed .thunder/angular/config.yaml). This keeps it idle on unrelated projects.
function cmdInit(root) {
  ...
  if (r.total === 0) {
    console.log(`thunder-angular: initialized (${cfg}) — no Angular sources found yet; the index will fill as you add them.`);
    return;
  }
  console.log(`thunder-angular: ${already ? 're-' : ''}initialized & indexed (${r.total} files) under .thunder/angular/ — commit it to share. Run /thunder-angular:thunder-angular-reindex to infer the functional layer.`);
}

function cmdEnsure(root) {
  if (!isInitialized(root)) return; // opt-in: idle until `thunder-angular-init` runs on this project
  drainDirty(root);
  const r = build(root);
  if (r.total === 0) return;
...
function cmdResetFunctional(root) {
  const p = join(root, '.thunder', 'angular', 'functional.json');
...
function cmdAsk(root, query, topOverride, factsMode = false) {
  ...
  if (!scored.length) {
    const brief = join(root, '.thunder', 'angular', 'project-brief.yaml');
...
function cmdAskDetail(root, ctxId) {
  ...
  const p = join(root, '.thunder', 'angular', 'projects', proj || '', (feat || '') + '.yaml');
