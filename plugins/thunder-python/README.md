...
thunder-python stays **completely idle** on a project until you opt it in — no `.thunder/` directory, no tokens — so it never touches unrelated repos. The skills answer **only** from a pre-built index. **Enable it here once:**

```
/thunder-python:thunder-python-init      # opt this project in + build the technical index
/thunder-python:thunder-python-reindex   # (optional) infer the functional/business layer
```

`init` writes the committed marker `.thunder/python/config.yaml` (its presence is what enables the hooks — commit it to share). The **technical** layer builds free & instantly; the **functional/business** layer is inferred once by
