...
const sample = [...model.contexts].sort((a, b) => b.routes.length - a.routes.length).find((c) => c.routes.length) || model.contexts[0];
...
const Q = [
  { kind: 'archi', struct: true, A: () => fileTok(join(C, 'project-brief.yaml')), B: () => fileTok(...allPy), Cc: () => SUBAGENT + fileTok(shard(sample)) },
