---
name: thunder-node-grok
description: Answer a question about what a Node.js backend does or how it works (user-facing or technical), token-minimally, by answering INLINE from thunder-node's index. Use for "how does the X endpoint work", "where is feature Y handled", "what does the orders feature do", "which routes exist", "trace what happens on /users".
allowed-tools: Read, Grep, Bash, Task
---

# grok — answer a question about the Node.js backend, INLINE
...
|---|---|
| "where is X defined", "who uses/injects X", "find the controller/service X" | `node "$ENG" sym def\|refs <Name> "$ROOT"` (~30 tok, exact) |
...
1. **Architecture / overview / "what does the app do" / list routes** → read **one** file:
   `Read .thunder/node/project-brief.yaml` (arch style, projects + roles, all routes, key
...
- If `purpose: null` / `functional_stale`, suggest `/thunder-node:thunder-node-reindex` first.
