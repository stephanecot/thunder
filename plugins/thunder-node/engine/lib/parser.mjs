...
// HTTP-verb method decorator (NestJS): @Get('x') / @Post() / @Delete(':id') …
const HTTP_DECO = /^@(Get|Post|Put|Delete|Patch|All|Options|Head)\b(?:\(\s*['"`]?([^'"`)]*)['"`]?\s*\))?/i;
const httpVerbDeco = (decs) => { for (const d of decs) { const m = d.match(HTTP_DECO); if (m) return { verb: m[1].toUpperCase(), sub: (m[2] || '').trim() }; } return null; };
// Join a controller base path + a method sub-path into one normalized route ("/users", "users/:id" → "/users/:id").
const joinPath = (base, sub) => ('/' + [base, sub].filter(Boolean).join('/')).replace(/\/{2,}/g, '/').replace(/(.)\/$/, '$1');

/** Express / Fastify functional routes: `app.get('/x', h)`, `router.post('/y', h)`, `fastify.get('/z', …)`. */
function extractCallRoutes(cleanLines, rawLines) {
  // Detect the call STRUCTURE on the cleaned line (so it's real code, not a comment), then read the
  // path LITERAL from the raw line (the lexer blanks string contents in the cleaned line).
  const CALL = /\b([A-Za-z_$][\w$]*)\s*\.\s*(get|post|put|delete|patch|all|options|head)\s*\(/;
  const ROUTER = /^(app|router|server|fastify|api|route|r|v\d+)$/i;
  const out = [];
  for (let i = 0; i < cleanLines.length; i++) {
    const m = cleanLines[i].match(CALL);
    if (!m) continue;
    const [, recv, verb] = m;
    const raw = rawLines[i] || '';
    const pm = raw.match(new RegExp(`\\b${recv}\\s*\\.\\s*${verb}\\s*\\(\\s*[\`'"]([^\`'"]+)[\`'"]`, 'i'));
    const path = pm ? pm[1] : null;
    if (!path) continue;
    if (!ROUTER.test(recv) && !path.startsWith('/')) continue; // avoid map.get(key) etc.
    // a NAMED handler is a bare identifier as the last arg (`…, usersController.create)`); an inline
    // `(req, res) => …` arrow has no name → `(inline)` (don't capture the `res` param).
    const hm = raw.replace(/\s+$/, '').match(/,\s*([A-Za-z_$][\w$.]*)\s*\)\s*;?$/);
    out.push({ verb: verb.toUpperCase(), path, target: hm ? hm[1] : '(inline)', kind: 'endpoint', line: i + 1 });
  }
  return out;
}

/** Per-file framework signals (aggregated per context in derive). */
function detectFw(raw) {
  const fw = [];
  if (/@nestjs\/|@Controller\b|@Injectable\b|@Module\b|@(Get|Post|Put|Delete|Patch)\s*\(/.test(raw)) fw.push('nestjs');
  if (/from\s+['"]express['"]|require\(\s*['"]express['"]|express\s*\(\)|\bRouter\s*\(\)/.test(raw)) fw.push('express');
  if (/from\s+['"]fastify['"]|require\(\s*['"]fastify['"]|\bfastify\s*\(/.test(raw)) fw.push('fastify');
  return [...new Set(fw)];
}
...
export function parseFile(raw, relPath) {
  ...
  for (let li = 0; li < cleanLines.length; li++) {
    ...
    if (tm) {
      ...
      const ctrlDec = pending.find((d) => /^@Controller\b/.test(d));
      const basePath = ctrlDec ? (ctrlDec.match(/@Controller\(\s*['"`]([^'"`]*)['"`]/) || [, ''])[1] : '';
      const type = {
        kind: tm[1], name: tm[2], decorators: pending.slice(), line: li + 1,
        ext: ext ? ext[1].trim() : null, impls: impl ? impl[1].trim() : null,
        methods: [], props: [], ctorDeps: [], basePath,
      };
      ...
      if (ctorAt >= 0) {
        ...
        if (member) {
          ...
          if (member.kind === 'prop') {
            ...
            const hv = httpVerbDeco(decs); // NestJS @Get/@Post/… → an HTTP endpoint
            if (hv) endpoints.push({ verb: hv.verb, path: joinPath(top.basePath, hv.sub), target: `${top.name}.${member.name}`, kind: 'endpoint', line: li + 1 });
          }
          consumed = true;
        }
      }
  ...
  // routes = NestJS decorator endpoints + Express/Fastify call routes (deduped by verb+path)
  const seen = new Set();
  file.routes = [...endpoints, ...extractCallRoutes(cleanLines, rawLines)]
    .filter((r) => { const k = r.verb + ' ' + r.path; if (seen.has(k)) return false; seen.add(k); return true; });
  file.functionals = []; // no Angular functional guards in Node
  file.fw = detectFw(raw);
  return file;
}
