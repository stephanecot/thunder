...
function cmdResetFunctional(root) {
  const p = join(root, '.claude', 'cache', 'thunder-node', 'functional.json');
...
function cmdAsk(root, query, topOverride, factsMode = false) {
  ...
  if (!scored.length) {
    const brief = join(root, '.claude', 'cache', 'thunder-node', 'project-brief.yaml');
...
function cmdAskDetail(root, ctxId) {
  ...
  const p = join(root, '.claude', 'cache', 'thunder-node', 'projects', proj || '', (feat || '') + '.yaml');
