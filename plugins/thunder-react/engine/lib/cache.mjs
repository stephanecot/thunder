...
export const cacheDir = (root) => join(root, '.thunder', 'react');

/** One-time migration: drop the pre-relocation derived index (.claude/cache/thunder-react/).
 * Projects indexed before the move to .thunder/react/ keep a stale copy there that masquerades
 * as the live index — remove it on every build. */
export function sweepLegacyCache(root) {
  for (const name of ['thunder-react']) {
