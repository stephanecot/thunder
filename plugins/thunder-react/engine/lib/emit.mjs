...
const routeStr = (r) => `${r.path || '/'} → ${r.target || r.kind}`;

/** Detect the React stack (deterministic, no LLM). */
function frameworkSummary(model) {
  const fws = [...new Set(model.contexts.map((c) => c.framework).filter((fw) => fw && fw !== 'node'))].sort();
  return fws.length ? fws.join(' + ') : 'React';
}
...
export function emit(root, model, functional = {}) {
  ...
  // global route table — ONE grep-friendly line per route (was 4-5 lines each; flow lives in shards)
  const rtLine = (r) => `${r.path || '/'} → ${r.target || ''}`.trimEnd() + `  ${r.kind}  (${r.ctx})`;
  if (writeIfChanged(join(dir, 'routes.yaml'),
    dump({ format: 'path → Target  kind  (context)', routes: model.routes.map(rtLine) }))) changed++;
  ...
  for (const c of model.contexts) {
    ...
    const shard = {
      context: {
        ...
        components: c.components.map((cp) => ({ n: cp.n, kind: cp.kind, ...(cp.props ? { props: cp.props } : {}), ...(cp.hooks ? { hooks: cp.hooks } : {}), ...(cp.deps ? { deps: cp.deps } : {}) })),
        services: c.services,
