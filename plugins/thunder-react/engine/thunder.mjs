...
function cmdEnsure(root) {
  ...
  const r = build(root);
  if (r.total === 0) return;
  console.log(`thunder-react: index frais (${r.model.projects.length} projets, ${r.model.contexts.length} contextes, ${r.model.routes.length} routes). /thunder-react:thunder-react-codemap pour explorer.`);
}
...
function cmdStale(root, asJson) {
  ...
  if (!stale.length) { console.log('thunder-react: couche fonctionnelle à jour'); return; }
  console.log(`thunder-react: ${stale.length} contexte(s) à (ré)inférer :`);
...
function cmdStaleModules(root, asJson) {
  ...
  if (asJson) { console.log(JSON.stringify(stale)); return; }
  if (!stale.length) { console.log('thunder-react: project themes up to date'); return; }
  for (const s of stale) console.log(`  ${s.module}  [${s.reason}]`);
}

function cmdResetFunctional(root) {
  const p = join(root, '.claude', 'cache', 'thunder-react', 'functional.json');
  try { if (existsSync(p)) rmSync(p); } catch { /* ignore */ }
  build(root);
  console.log('thunder-react: couche fonctionnelle réinitialisée');
}
...
async function cmdSetFunctional(root, ctxId) {
  ...
  build(root);
  console.log(`thunder-react: fonctionnel mis à jour pour ${ctxId} (evidence_hash ${entry.evidence_hash})`);
}
...
async function cmdSetModuleFunctional(root, name) {
  ...
  build(root);
  console.log(`thunder-react: project theme updated for ${name}${entry.theme ? ` ("${entry.theme}")` : ''}`);
}
...
function cmdAsk(root, query, topOverride, factsMode = false) {
  ...
  if (!scored.length) {
    const brief = join(root, '.claude', 'cache', 'thunder-react', 'project-brief.yaml');
...
function cmdAskDetail(root, ctxId) {
  ...
  const p = join(root, '.claude', 'cache', 'thunder-react', 'projects', proj || '', (feat || '') + '.yaml');
...
async function selftest() {
  ...
  assert.ok(model.routes.length >= 3, 'React Router routes found');
  assert.ok(route('/users') && route('/cart'), 'routes /users and /cart');

  // function components + custom hooks
  const users = ctx('features.users');
  assert.ok(users, 'users feature context');
  assert.equal(users.framework, 'react', 'react detected');
  const list = users.components.find((c) => c.n === 'UserList');
  assert.ok(list && list.kind === 'function', 'UserList function component');
  assert.deepStrictEqual(list.deps, ['useUsers'], 'component uses the useUsers hook');
  assert.ok(users.services.useUsers, 'useUsers custom hook (service)');
  assert.ok(users.services.useUsers.hooks.includes('useState') && users.services.useUsers.hooks.includes('useEffect'), 'hook uses useState + useEffect');

  const cart = ctx('features.cart');
  assert.ok(cart.components.find((c) => c.n === 'Cart'), 'Cart component (arrow)');
  assert.ok(cart.services.useCart, 'useCart hook');

  // flow derived from route → component → custom hooks
  const r = route('/users');
  assert.match(r.flow, /UserList → useUsers/, `flow: ${r.flow}`);

  console.log('✅ selftest OK —', model.routes.length, 'routes,', model.contexts.length, 'contextes, framework: react');
}
