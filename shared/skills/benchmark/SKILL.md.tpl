---
name: thunder-__LANG__-benchmark
...
This skill (re)generates `BENCHMARK.md` for **thunder-__LANG__** so every plugin's report shares the
...
```
cd <plugin>
node engine/thunder.mjs build __REALDEMO__ --force      # realistic bench bed (generate it first if missing: __GENCMD__)
...
```
node engine/tools/token-bench.mjs __REALDEMO__          # two-tier / inline A·B·C — data tokens
node engine/tools/sweep-bench.mjs __REALDEMO__          # ≥50 routed questions → wins/total + % saved
...
2. **Symbol** — `node engine/thunder.mjs sym def <Type> __REALDEMO__` and `sym refs <Type>`  vs grep+open.
3. **Discovery** — `grep -i <term> <cache>/capability-map.yaml`  vs reading the repo.
4. **Flow + rules** — `node engine/thunder.mjs ask "<feature> flow" __REALDEMO__`  vs reading the feature.
...
`<cache>` = `__REALDEMO__/.claude/cache/thunder-__LANG__`.
...
```markdown
# thunder-__LANG__ — benchmark report (before / after)
...
**Test beds.** `demo` (toy) · `__REALDEMO__` (realistic, main bed) · `__BIGDEMO__` (extreme scale).

## 1. Results table — __REALDEMO__
...
## 10. Rerun
\`\`\`
node engine/thunder.mjs build __REALDEMO__ --force && node engine/tools/token-bench.mjs __REALDEMO__
node engine/tools/sweep-bench.mjs __REALDEMO__ && node engine/tools/tier3-bench.mjs demo
\`\`\`
```
