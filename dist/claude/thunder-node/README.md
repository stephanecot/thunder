...
Claude Code plugin for **token-minimal comprehension / exploration / navigation** of a Node.js backend codebase
(Express / Fastify / NestJS). Pure **Node.js indexing engine, zero dependencies**, cross-platform. Shares the
thunder engine architecture (WALK → LEX → PARSE → DERIVE → EMIT, NDJSON cache, sharded YAML index,
inferred functional layer, hooks) with a **TS/JS lexer**, a **controller + endpoint + framework parser**, and
**Node-semantic derivation**.
...
| Type | Name | Role |
|---|---|---|
| Skill | `/thunder-node:thunder-node-codemap` | explore via the index (projects, features, routes, components, services) |
| Skill | `/thunder-node:thunder-node-sym` | locate a symbol (def / refs / DI usages) without reading sources |
| Skill | `/thunder-node:thunder-node-reindex` | refresh the index; re-infer meaning (budgeted); `--full` / `--tech` |
| Skill | `/thunder-node:thunder-node-grok` | answer a feature/technical question (capped, seeded fan-out) |
...
/plugin install thunder-node@thunder
```
