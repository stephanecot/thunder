---
name: thunder-react-codemap
description: 'Explore and understand a React.js codebase (TS/JSX) token-minimally by answering INLINE from thunder-react''s pre-built YAML index (projects, feature contexts, routes, components, services, dependency-injection graph, user-facing meaning). Use whenever the user asks how the app is structured, where a component/hook lives, what routes exist, or what a feature does — instead of reading .tsx/.ts/.jsx/.js files.'
---

# codemap — understand the React app, INLINE

thunder-react maintains a YAML index under `<project>/.claude/cache/thunder-react/`. Answer from it **in
...
|---|---|
| "where is X defined", "who uses/injects X", "find the component/hook X" | `thunder.mjs sym def\|refs <Name>` (~30 tok, exact) |
...
2. **A specific feature / where / flow / rule** → **one** command:
   `node "${PLUGIN_ROOT}/engine/thunder.mjs" ask "<keywords>" "${PWD}"`
...
## Notes
- `functional_stale: true` or `purpose: null` → suggest `/thunder-react-reindex`.
- For a precise symbol (component/hook def or references): `/thunder-react-sym`.
