...
> **Prerequisite:** opt the project in first — run `/thunder-angular-init` once (it writes the committed `.thunder/angular/config.yaml` marker). Running this skill also writes that marker when it builds a non-empty index, so reindex works standalone too.
...
4. **Infer in batches** — split `contexts` into groups of **~10**. For each group spawn ONE
   **thunder-angular-cartographer** (the @thunder-angular-cartographer agent), passing only the ids + paths of that group:
