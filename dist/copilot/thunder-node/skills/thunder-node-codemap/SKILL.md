...
thunder-node maintains a YAML index under `<project>/.thunder/node/`. Answer from it **in
...
## Notes
- `functional_stale: true` or `purpose: null` → suggest `/thunder-node-reindex`.
- For a precise symbol (controller/service def or references): `/thunder-node-sym`.
